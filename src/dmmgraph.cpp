...
DMMGraph::exportDataSLOT()
{
  ...
  {                 
    ...
    {
      QDateTime dt = m_graphStartDateTime.addSecs( qRound(delta_t/10.0) );
      delta_t += m_sampleTime;
      QString line;
      line.sprintf( "%02d.%02d.%04d\t%02d:%02d:%02d.%01d\t%06g\t%s\n",
          dt.date().day(), dt.date().month(), dt.date().year(),
          dt.time().hour(), dt.time().minute(), dt.time().second(), (delta_t%10),
          (*m_array)[i],
          m_unit.latin1() );
