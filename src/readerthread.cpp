...
bool ReaderThread::open( const QString & device, BaudRateType baud, ParityType parity, 
												 DataBitsType bits, StopBitsType stopBits, bool rts, bool dtr )
{
	if (isOpen()) m_port->close();
  ...
  m_port->setStopBits( stopBits );
	...
	//m_notifier = new QSocketNotifier( m_port, QSocketNotifier::Read );
...
void
ReaderThread::socketNotifierSLOT()
{	
...
ReaderThread::checkFormat()
{
  ...
  else if (m_format == ReadEvent::M9803RContinuous && bytes_read >= 10)
  {
    if (m_fifo[(m_length-1+FIFO_LENGTH)%FIFO_LENGTH] == 0x0d && 
        m_fifo[m_length] == 0x0a) return true; 
  }
  else if (m_format == ReadEvent::VC940Continuous && bytes_read >= 11)
  {
    if (m_fifo[(m_length-1+FIFO_LENGTH)%FIFO_LENGTH] == 0x0d && 
        m_fifo[m_length] == 0x0a) return true; 
  }
  else if (m_format == ReadEvent::PeakTech10 && bytes_read >= 11)
  {
  ...
  else if (m_format == ReadEvent::IsoTech && bytes_read >= 22)
  {
    for (int i=0; i<11; ++i)
    {
